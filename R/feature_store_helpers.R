...
drop_feature_store <- function(conn, pattern = NULL) {
  ...
  if ("logs" %in% tables_to_delete$table) {
    stopifnot("'fs.logs' set to delete. Can only delete if entire featurestore is dropped."={identical(tables_to_delete, dplyr::filter(mg_get_tables(conn), .data$schema == "fs"))})
  }
  ...
  if (mg_table_exists(conn, "fs.logs")) {
    purrr::walk(tables_to_delete$table, ~ DBI::dbExecute(conn, glue::glue("DELETE FROM fs.logs WHERE logs.table = '{.x}'")))
  }
}
