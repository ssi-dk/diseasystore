...
The `diseasystores` are [dynamically expanded](quick-start.html#dynamically-expanded), so feature computation is often
...
* `.data` is a grouped `data.frame` whose groups are those specified by the `stratification` argument
  (see [Automatic aggregation](quick-start.html#automatic-aggregation)).
...
* [DiseasystoreGoogleCovid19: index](https://github.com/ssi-dk/diseasystore/blob/1319eb93af/R/DiseasystoreGoogleCovid19.R#L111)

* [DiseasystoreGoogleCovid19: min temperature](https://github.com/ssi-dk/diseasystore/blob/1319eb93af/R/DiseasystoreGoogleCovid19.R#L203)
...
This latter part becomes important when [clean up](quick-start.html#dropping-computed-features) for the data base needs
to be performed.
...
For an example, you can consult [DiseasystoreGoogleCovid19: key_join_filter](https://github.com/ssi-dk/diseasystore/blob/1319eb93af/R/DiseasystoreGoogleCovid19.R#L258)

[^1]: The `SCDB` package places `checksum`, `from_ts`, and `until_ts` as the last columns. But `valid_from` and `valid_until` should be the last columns in the output passed to `SCDB`.
