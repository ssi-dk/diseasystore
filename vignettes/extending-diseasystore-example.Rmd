...
the individuals throughout the period[^3]. Each birth day has an associated age and a validity period which runs from
...
With the `target_conn` option set, we can easily create a new instance of the `diseasystore`[^5].
...
[^5]: The `DiseasystoreSimulist` is bundled with the package, but not exported. Therefore, we use the `:::` operator to
retrieve the class.
