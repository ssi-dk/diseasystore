...
```{r google_setup, eval = FALSE, eval = suggests_available && data_available && not_on_cran}
...
```{r google_available_features, eval = suggests_available && data_available && not_on_cran}
...
```{r google_get_feature_example_1, eval = suggests_available && data_available && not_on_cran}
...
```{r google_get_feature_example_2, eval = suggests_available && data_available && not_on_cran}
ds$get_feature("n_hospital",
               start_date = as.Date("2020-03-01"),
...
```{r google_key_join_features_example_1, eval = suggests_available && data_available && not_on_cran}
...
```{r google_key_join_features_example_2, eval = suggests_available && data_available && not_on_cran}
ds$key_join_features(observable = "n_hospital",
                     stratification = rlang::quos(country_id))
```

The `stratification` argument is very flexible, so we can supply any valid R expression:
```{r google_key_join_features_example_3, eval = suggests_available && data_available && not_on_cran}
ds$key_join_features(observable = "n_hospital",
                     stratification = rlang::quos(country_id,
                                                  old = age_group == "90+"))
```
...
```{r drop_diseasystore_example_1, eval = suggests_available && data_available && not_on_cran}
SCDB::get_tables(ds$target_conn)
```

```{r drop_diseasystore_example_2, eval = suggests_available && data_available && not_on_cran}
