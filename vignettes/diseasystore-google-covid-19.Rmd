...
```{r configure_diseasystore_hidden, include = FALSE, eval = suggests_available && data_available && not_on_cran}
target_conn <- \() DBI::dbConnect(RSQLite::SQLite())
if (rlang::is_installed("withr")) {
...
```{r initializing_diseasystore, eval = suggests_available && data_available && not_on_cran}
ds <- DiseasystoreGoogleCovid19$new()
```

Once configured such, we can also use the feature store directly to get data.
```{r using_diseasystore_example_1, eval = suggests_available && data_available && not_on_cran}
# We can see all the available features in the feature store
ds$available_features
```

```{r using_diseasystore_example_2, eval = suggests_available && data_available && not_on_cran}
