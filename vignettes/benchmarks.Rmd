...
```{r, echo = FALSE, eval = rlang::is_installed("here") && rlang::is_installed("RSQLite")}
...
# Add note on "bugged" backends:
# For some backends, the first iteration is much slower.
# It seem the pre-computing did not work in these cases.
# We remove these first iterations from the analysis
bugged_timings <- benchmarks |>
  dplyr::group_by(dplyr::across(c("benchmark_function", "database", "version", dplyr::all_of(groups)))) |>
  dplyr::mutate("rel_time" = 1) |> #abs(.data$time / median(.data$time))) |>
  dplyr::summarize(bugged = any(.data$rel_time > 2 | .data$rel_time < 0.5), .groups = "drop") |>
  dplyr::filter(.data$bugged) |>
  dplyr::select(!"bugged")

benchmarks <- benchmarks |>
  dplyr::mutate(r = dplyr::row_number(), .by = colnames(bugged_timings)) |>
  dplyr::anti_join(dplyr::mutate(bugged_timings, r = 1), by = c(colnames(bugged_timings), "r")) |>
  dplyr::select(!"r")
