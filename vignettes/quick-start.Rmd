...
The `diseasystores` uses `SCDB` in the backed and can use any data base back end supported by `SCDB`.

```{r google_setup_hidden, include = FALSE}
# We assume the data is made available to us in a "vignette_data" folder
# If it isn't, and we have an internet connection, we download some of the Google COVID-19 data for this vignette
google_files <- c("by-age.csv", "demographics.csv", "index.csv", "weather.csv")
local_conn <- "vignette_data"
remote_conn <- diseasyoption("remote_conn", "DiseasystoreGoogleCovid19")

# Check that the files are available
vignette_data_missing <- purrr::some(paste0(google_files, ".gz"), ~ !file.exists(file.path(local_conn, .)))

if (vignette_data_missing && curl::has_internet()) {
  ...
  purrr::keep(google_files, ~ !file.exists(file.path(local_conn, paste0(., ".gz")))) |>
    purrr::walk(\(file) {
      ...
      R.utils::gzip(file.path(local_conn, file))
    })
}

# Check that the files are available after attempting to download
if (purrr::some(google_files, ~ !file.exists(file.path(local_conn, paste0(., ".gz"))))) {
...
```{r google_get_feature_example_1}
...
```{r google_get_feature_example_2}
ds$get_feature("n_hospital",
               start_date = as.Date("2020-03-01"),
...
That is, say a given "n_hospital" has been computed between 2020-03-01 and 2020-03-15. The call `$get_feature("n_hospital", start_date = as.Date("2020-03-01"), end_date = as.Date("2020-03-20")` only needs to compute the feature between 2020-03-16 and 2020-03-20.
...
That is, if a feature has been computed for a time interval on one `slice_ts`, `diseasystore` will recompute the feature for any other `slice_ts`. 
...
```{r google_key_join_features_example_1}
...
```{r drop_diseasystore_example_1}
SCDB::get_tables(ds$target_conn)
```

```{r drop_diseasystore_example_2}
...
options("diseasystore.source_conn" = "/local/file/path")
diseasyoption("source_conn", class = "DiseasystoreDiseaseY")
```
